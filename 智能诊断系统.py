...
def test_lesson_plan(text):
    """调用大模型检测3个维度"""
    prompt = f"""
你是初中信息科技教案诊断专家，分析以下教案，输出3部分结果：

1. 教学环节完整性（总分100分，100=完全合格）
   检查项：必须包含【导入、新授、练习、总结、作业】5个环节，每个环节需满足：  
   存在性：环节名称明确（如"导入""新授"等，含变体如"引入""讲解"需映射到标准环节）；  
   内容有效性：环节描述≥30字，且含具体操作（如"导入用微信红包案例提问"而非"导入新课"）。  
   评分逻辑：  
...
2. 时间分配合理性（总分100分，100=完全合理）
   ...
   作业：1-3分钟（±1分钟）→ 合理范围0-4分钟。  
   评分逻辑：  
   特殊规则：若环节缺失（上述第1部分判定），'是否合理'必须为false，'建议时长'标注"环节缺失"
   每有1个环节时长超出参考范围扣15分；  
   总时长（各环节之和）超出40±5分钟（35-45分钟）扣20分；  
   扣完为止，最低0分。  
   输出字段：  
   'score'：最终得分（0-100）；  
   '建议'：每个环节的建议列表，含：  
     '环节'：标准环节名；  
     '当前时长'：从教案中提取的分钟数（若缺失则为0）；
     '建议时长'：参考范围（如"3-7分钟"）或"环节缺失"；  
     '是否合理'：true/false（仅当环节存在且时长在范围内时为true）。
...
3. 核心素养匹配度（基于参照案例的语义相似度加权计算）
   ...
   "环节完整性": {{
    "score": 70,
    "详情": "作业环节缺失（0分），练习环节内容简略仅20字（10分），导入新授总结环节完整有效（60分），总分70分",
    "各环节状态": [
      {{"环节": "导入", "得分": 20, "是否存在": true, "内容有效": true, "摘要": "用热门网站案例提问域名解析原理"}},
      {{"环节": "新授", "得分": 20, "是否存在": true, "内容有效": true, "摘要": "讲解域名DNS概念与递归/迭代查询原理"}},
      {{"环节": "练习", "得分": 10, "是否存在": true, "内容有效": false, "摘要": "完成教材练习题"}},
      {{"环节": "总结", "得分": 20, "是否存在": true, "内容有效": true, "摘要": "回顾DNS查询过程与编码转换要点"}},
      {{"环节": "作业", "得分": 0, "是否存在": false, "内容有效": false, "摘要": "环节缺失"}}
    ]
  }},
  "时间分配": {{
    "score": 85,
    "建议": [
      {{"环节": "导入", "当前时长": 5, "建议时长": "3-7分钟", "是否合理": true}},
      {{"环节": "新授", "当前时长": 25, "建议时长": "10-25分钟", "是否合理": true}},
      {{"环节": "练习", "当前时长": 8, "建议时长": "5-18分钟", "是否合理": true}},
      {{"环节": "总结", "当前时长": 2, "建议时长": "1-5分钟", "是否合理": true}},
      {{"环节": "作业", "当前时长": 0, "建议时长": "环节缺失", "是否合理": false}}
    ]
  }},
   "素养匹配": {{
        "avg_score": 82.25,
        "各素养": {{
            "信息意识": {{
                ...
                "相似度计算过程": {{
                    ...
                    "基础锚点匹配度": 0.60,
                    "缺失锚点匹配度": 0.10,
                    "权重计算公式": "0.85×0.5 + 0.60×0.3 + (1-0.10)×0.2 = 0.785 → 78.50分"
                }},
                "匹配证据": [
                    "优秀匹配：教案写'对比官方与自媒体信息' ↔ 锚点'对比官方媒体与自媒体'",
                    "基础匹配：教案写'培养甄别能力' ↔ 锚点'培养信息甄别意识'",
                    "差异点：教案未明确'记录判断依据'（优秀锚点要素）"
                ],
                "理由": "教案具备完整的信息对比活动（高相似度），但在元认知层面（记录依据）不足，故得78.50分而非满分"
            }},
            "计算思维": {{
                "分数": 85.00,
                "相似度计算过程": {{
                    ...
                    "基础锚点匹配度": 0.50,
                    "缺失锚点匹配度": 0.05,
                    "权重计算公式": "0.90×0.5 + 0.50×0.3 + (1-0.05)×0.2 = 0.45 + 0.15 + 0.19 = 0.79 → 79.00分→调整至85分（因具体任务分解明确）"
                }},
                "匹配证据": [
                    "教案明确提到步骤分解与流程图绘制（对应优秀锚点）"
                ],
                "理由": "具备明确的问题分解和算法描述活动"
            }},
            "数字化学习与创新": {{
                ...
                "匹配证据": ["使用Word制作小报（基础匹配），但无在线协作（缺失优秀要素）"],
                "理由": "基础工具应用充分，但缺乏协作创新元素"
            }},
            "信息社会责任": {{
                "分数": 92.00,
                "相似度计算过程": {{
...
教案文本：{text}
【强制要求】
1. 必须输出严格JSON格式，不要添加markdown代码块标记（```json）
2. "各环节状态"必须包含5个环节，每个必须有"得分"字段（20/10/0）
3. "时间分配"中缺失环节的"建议时长"必须是"环节缺失"，"是否合理"必须是false
4. 根据实际教案内容填写，不要复制示例数据
"""
