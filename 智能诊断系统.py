...
2. 时间分配合理性（总分100分，100=完全合理）
   ...
   评分逻辑：  
   特殊规则：若上述第1部分判定环节存在为false，则'是否合理'必须为false，'建议时长'标注"环节缺失"
   常规规则：
   每有1个环节时长超出参考范围扣15分；  
...
3. 核心素养匹配度（基于参照案例的语义相似度加权计算）
   ...
   - 缺失锚点差异：教案未提及"记录判断依据"（优秀锚点特有）
      ...
      输出严格JSON格式（示例，不要完全复制输出））：
   {{
   "环节完整性": {{
       "score": 70,
       "详情": "缺失'作业'环节（扣20分），'练习'环节内容仅25字（扣10分），总分70分",
       "缺失环节": ["作业"],
       "各环节状态": [
           {{"环节": "导入", "得分": 20, "是否存在": true, "内容有效": true, "摘要": "用微信红包案例提问数据概念"}},
           {{"环节": "新授", "得分": 20, "是否存在": true, "内容有效": true, "摘要": "讲解变量定义与数据类型，演示赋值操作"}},
           {{"环节": "练习", "得分": 10, "是否存在": true, "内容有效": false, "摘要": "完成教材练习1-3"}},
           {{"环节": "总结", "得分": 20, "是否存在": true, "内容有效": true, "摘要": "回顾变量命名规则与数据类型"}},
           {{"环节": "作业", "得分": 0, "是否存在": false, "内容有效": false, "摘要": "环节缺失"}}
       ]
   }},
   "时间分配": {{
       "score": 70,
       "详情":"'新授'环节时长不合理（扣15分），'作业'环节时长不合理（扣15分），总分70份"，
       "建议": [
           {{"环节": "导入", "当前时长": 5, "建议时长": "3-7分钟", "是否合理": true}},
           {{"环节": "新授", "当前时长": 28, "建议时长": "10-25分钟", "是否合理": false}},
