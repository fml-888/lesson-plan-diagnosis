...
1. 教学环节完整性（总分100分，100=完全合格）
   ...
   '缺失环节'：缺失的标准环节名称列表（如["作业", "总结"]）；  
   '各环节状态'：每个环节的状态列表，含：  
   '环节'：标准环节名（如"导入"）；  
   '是否存在'：true/false（名称是否明确）；  
   '内容有效'：true/false（字数≥30字且有具体操作）；  
   '摘要'：环节核心内容概括（≤50字）。

2. 时间分配合理性（总分100分，100=完全合理）
   检查项：总课时40分钟，各环节参考时长：  
   导入：3±2分钟（3-5分钟）；新授：15±5分钟（15-20分钟）；  
   练习：12±3分钟（8-15分钟）；总结：3±1分钟（2-4分钟）；作业：2±1分钟（1-3分钟）。  
   评分逻辑：  
   每有1个环节时长超出参考范围扣15分；  
   总时长（各环节之和）超出40±5分钟（40-45分钟）扣20分；  
   扣完为止，最低0分。  
   输出字段：  
   'score'：最终得分（0-100）；  
   '建议'：每个环节的建议列表，含：  
...
3. 核心素养匹配度（基于参照案例的语义相似度加权计算）
   ...
   "环节完整性": {{
       "score": 80,
       "详情": "缺失'作业'环节扣20分，'练习'环节内容仅25字扣10分",
       "缺失环节": ["作业"],
       "各环节状态": [
           {{"环节": "导入", "是否存在": true, "内容有效": true, "摘要": "用微信红包案例提问数据概念"}},
           {{"环节": "新授", "是否存在": true, "内容有效": true, "摘要": "讲解变量定义与数据类型，演示赋值操作"}},
           {{"环节": "练习", "是否存在": true, "内容有效": false, "摘要": "完成教材练习1-3"}},
           {{"环节": "总结", "是否存在": true, "内容有效": true, "摘要": "回顾变量命名规则与数据类型"}},
           {{"环节": "作业", "是否存在": false, "内容有效": false, "摘要": ""}}
       ]
   }},
   "时间分配": {{
       "score": 70,
       "建议": [
           {{"环节": "导入", "建议时长": "3-7分钟", "是否合理": true}},
           {{"环节": "新授", "建议时长": "20-30分钟", "是否合理": false}},
           {{"环节": "练习", "建议时长": "7-13分钟", "是否合理": true}},
           {{"环节": "总结", "建议时长": "2-4分钟", "是否合理": true}},
           {{"环节": "作业", "建议时长": "1-3分钟", "是否合理": true}}
       ]
   }},
   "素养匹配": {{
        "avg_score": 82.25,
