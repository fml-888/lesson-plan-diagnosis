...
2. 时间分配合理性（总分100分，100=完全合理）
   ...
   总时长（各环节之和）超出40±5分钟（35-45分钟）扣15分；  
   扣完为止，最低0分。 
...
4. 字数统计包含该环节下所有具体描述
  ...
  "时间分配": {{
       "score": 65,
       "详情":"'新授'环节时长28分钟超出范围（扣10分），'作业'环节时长5分钟超出范围（扣10分），总时长51分钟超出范围（扣15分）其余环节时间合理，总分65分"，
       "建议": [
           {{"环节": "导入", "当前时长": 4, "建议时长": "1-7分钟", "是否合理": true}},
           {{"环节": "新授", "当前时长": 28, "建议时长": "10-25分钟", "是否合理": false}},
           {{"环节": "练习", "当前时长": 11, "建议时长": "5-18分钟", "是否合理": true}},
           {{"环节": "总结", "当前时长": 3, "建议时长": "1-5分钟", "是否合理": true}},
           {{"环节": "作业", "当前时长": 5, "建议时长": "1-4分钟", "是否合理": false}}
       ]
   }},
   "素养匹配": {{
        "avg_score": 82.25,
